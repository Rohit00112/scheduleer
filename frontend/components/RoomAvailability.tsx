...
export default function RoomAvailability({ schedules }: RoomAvailabilityProps) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-4">Room Availability</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                {allRooms.map((room) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`font-semibold ${occupied ? "text-red-800" : "text-green-800"}`}>
                                {room}
                            </div>
                            {occupied ? (
            ...
            {allRooms.length === 0 && (
                <p className="text-sm text-gray-400 text-center py-4">No room data available.</p>
            )}
        </div>
    );
}
